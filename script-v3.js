...
// Review form logic
(() => {
  const tradesEl = document.getElementById('review-trades');
  const tradesVal = document.getElementById('review-trades-value');
  if (!tradesEl || !tradesVal) return;
  tradesEl.addEventListener('input', (e) => {
    tradesVal.textContent = e.target.value;
  });
})();
